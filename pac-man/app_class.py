...
class App:
    def __init__(self):
        ...
        self.currentpath = self.player.path.copy()
    ...
    def start_draw(self):
        ...
        self.draw_text('G - GENERATE NEW MAZE', self.screen, [
            WIDTH // 2, HEIGHT // 2 + 175], START_TEXT_SIZE, (44, 167, 198), START_FONT, centered=True)
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_1:
                    self.player.path = self.player.pathBFS.copy()
                    self.currentpath = self.player.pathBFS.copy()
                    self.state = 'playing'
                if event.key == pygame.K_2:
                    self.player.path = self.player.pathAStar.copy()
                    self.currentpath = self.player.pathAStar.copy()
                    self.state = 'playing'
                if event.key == pygame.K_3:
                    self.player.path = self.player.pathGreed.copy()
                    self.currentpath = self.player.path.copy()
                    self.state = 'playing'
                if event.key == pygame.K_SPACE:
                    self.state = 'start'
    ...
    def playing_update(self):
        ...
        if self.player.grid_pos == self.coins[0]:
            self.player.path = self.currentpath.copy()
            print("Iterations count: ", len(self.currentpath))
            self.reset()
    ...
    def generate(self):
        maze = [[1] * (COLS + 1) for i in range(ROWS + 1)]

        for i in range(len(maze)):
            for j in range(len(maze[i])):
                if i % 2 == 1 and j % 2 == 1 and i != ROWS and j != COLS:
                    maze[i][j] = 0
        ...
        for i in range(destroyable):
            ...
            if walllist[randw][0] == 0 or walllist[randw][1] == 0 or walllist[randw][0] == ROWS or walllist[randw][1] == COLS:
                i -= 1
